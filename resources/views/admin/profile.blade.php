...
<form action="{{route('update_profile')}}" enctype="multipart/form-data" id="editaccountsettings" method="post">
    @csrf
    <div class="row">
                ...
                <div class="card-body p-4">
                    <div class="text-center">
                        <div class="profile-user position-relative d-inline-block mx-auto  mb-4">
                                                <input type="hidden" id="cover_img_hidden" value="{{$user->cover_image ?? ''}}">
                                                @if (isset($user->cover_image) && !empty($user->cover_image))
                                                    @if (file_exists(public_path('/uploads/user_profile') . '/' . $user->cover_image))
                                                    
                                                    <img class="rounded-circle avatar-xl img-thumbnail user-profile-image" src="{{ url('public/uploads/user_profile') . '/' . $user->cover_image }}" alt="user-profile-image">
                                                    @else
                ...
                <div class="card-body p-4">
                    <div class="tab-content">
                                    ...
                                    <div class="col-lg-4">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" id="username"
                                                placeholder="Enter your username" name="username" value="{{$user->name ?? ''}}">
                                        </div>
                                    </div>
                                    <!--end col-->
                                    <div class="col-lg-4">
                                            ...
                                            <input type="text" class="form-control onlyNumerics" id="mobile_no"
                                                placeholder="Enter your phone number" name="mobile_no" value="{{$user->mobile_no ?? ''}}">
                                            </div>
                                    </div>
                                    <input type="hidden" id="user_hidden_id" value="{{$user->id ?? ''}}">
                        ...
                        <div class="tab-pane" id="changePassword" role="tabpanel">
                            <form action="{{route('change-password')}}" id="confirmnewpasswordform" method="POST">
                                @csrf
                                <div class="row g-2">
                                    <div class="col-lg-4">
                                        <div>
...
@section('script')
    ...
    @if( isset($user['country_flag']) && !empty($user['country_flag']) )
    phoneInput.setCountry("{{ $user['country_flag'] }}");
    @endif
