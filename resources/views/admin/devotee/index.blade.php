        ...
        <div class="col-lg-12">
            <div class="card" id="devoteeList">
                    ...
                    <div class="row g-4 align-items-center">
                        ...
                        <div class="col-sm-auto ms-auto">
                            <div class="hstack gap-2">
                                <div class="">
                                    {!! Form::select('assign_category_id',$category ?? '','',[ 'class'=>'form-control','id'=>'assign_category_id','placeholder'=>'Please Select','style'=>"display: none;"]) !!}
                                </div>
                                ...
                                @if (auth()->guard('admin')->user()->rights != 3)
                                    ...
                                    <button class="btn btn-outline-primary waves-effect waves-light export-btn" type="button" data-bs-toggle="offcanvas" data-bs-target="#exportModal" aria-controls="exportModal"><i
                                        class="ri-file-download-line align-bottom me-1"></i> @lang('translation.export')</button>
                                    <button class="btn btn-outline-primary waves-effect waves-light import-btn" type="button" data-bs-toggle="offcanvas" data-bs-target="#importModal" aria-controls="importModal"><i
                                        class="ri-file-upload-line align-bottom me-1"></i> @lang('translation.import')</button>
                                    <a href="{{route('devotee.add')}}" class="btn btn-success add-btn"><i
                                        class="ri-add-line align-bottom me-1"></i> Add @lang('translation.devotee')</a>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body">
                                ...
                                <tbody class="list form-check-all">
                                    @if(isset($devotee) && !empty($devotee))
                                        ...
                                        <tr>
                                                ...
                                                <div class="d-flex align-items-center">
                                                    <div class="flex-shrink-0">
                                                        @if (isset($val->image) && !empty($val->image))
                                                            @if (file_exists(public_path('/uploads/devotee_profile') . '/' . $val->image))
                                                            <img class="avatar-xxs rounded-circle image_src object-cover" src="{{ url('public/uploads/devotee_profile') . '/' . $val->image }}" alt="user-profile-image">
                                                            @else
                                                            <img class="avatar-xxs rounded-circle image_src object-cover" src="{{ url('public/build/images/users/user-dummy-img.png') }}" alt="user-profile-image">
                                                            @endif
                                                        @else
                                                            <img class="avatar-xxs rounded-circle image_src object-cover" src="{{ url('public/build/images/users/user-dummy-img.png') }}" alt="user-profile-image">
                                                        @endif
                                                    </div>
                                                    <div class="flex-grow-1 ms-2 name">{{ ucfirst($val->surname) }} {{ ucfirst($val->first_name) }} {{ucfirst($val->last_name)}}
                                                        @if(isset($val->category->name) && $val->category->name != "")
                                                            <span class="badge bg-success">{{ $val->category->name}}</span>
                                                        @endif
                                                        </div>
                                                </div>
                                            </td>
                                            ...
                                            <td>{{$val->area}}</td>
                                            <td>{{ isset($val->country->title) ? $val->country->title : "" }}</td>
                                            <td>{{ isset($val->state->title) ? $val->state->title : "" }}</td>
                                            <td>{{ isset($val->city->title) ? $val->city->title : "" }}</td>
                                            ...
                                            <td>
                                                <ul class="list-inline hstack gap-2 mb-0">
                                                    <li class="list-inline-item" data-bs-toggle="tooltip"
                                                        ...
                                                        title="View">
                                                        <a class="edit-item-btn" href="{{route('devotee.view',$val->id)}}"><i
                                                    ...
                                                    <li class="list-inline-item" data-bs-toggle="tooltip"
                                                        data-bs-trigger="hover" data-bs-placement="top"
                                                        title="Edit">
                                                        <a class="edit-item-btn" href="{{route('devotee.edit',$val->id)}}"><i
                                                                class="ri-pencil-fill align-bottom text-muted"></i></a>
                                                    </li>
                                                    <li class="list-inline-item" data-bs-toggle="tooltip"
                                                        data-bs-trigger="hover" data-bs-placement="top"
                                                        title="Delete">
                                                        <a class="remove-item-btn" onclick="dms_modal.confirmModal('{{ url($actionURL.'/action/delete/'.$val->id) }}');">
                                                            <i
                    ...
                    <div class="modal fade zoomIn" id="assignCategoryModal" tabindex="-1"
                        aria-labelledby="assignCategoryLabel" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                ...
                                <form action="{{ url($actionURL.'/multiple_assign_category/') }}" id="assign_form" method="post">
                                    @csrf
                                    <input type="hidden" id="multiple_userid" name="assignuser" value="">
                                    <input type="hidden" id="cat_id" name="cat_id" value="">

                                    <div class="modal-body p-5 text-center">
                                        <div class="mt-4 text-center">
                                            <h4 class="fs-semibold">You are about to category a devotee?</h4>
                                            <p class="text-muted fs-14 mb-4 pt-1">your devotee will
                                                assign category.</p>
                                            <div class="hstack gap-2 justify-content-center remove">
                        ...
                        <form action="" class="d-flex flex-column justify-content-end h-100">
                            <div class="offcanvas-body">
                                <div class="mb-4">
                                    ...
                                    <input type="text" name="name" value="{{ $filter['name'] ?? '' }}" class="form-control" id="first_name" placeholder="Serach By First Name">
                                </div>
                                <div class="mb-4">
                                    <label for="first_name"
                                        class="form-label text-muted text-uppercase fw-semibold mb-3">First Name</label>
                                ...
                                <div class="mb-4">
                                    <label for="area" class="form-label text-muted text-uppercase fw-semibold mb-3">Category</label>
                                    {!! Form::select('category[]',$category ?? '',$filter['category'] ?? '',[ 'class'=>'form-control','id'=>'category','data-select2init'=>'true','multiple'=>'true','data-placeholder'=>'Please Select']) !!}
                                </div>
                                <div class="mb-4">
                                    <label for="email"
                                        class="form-label text-muted text-uppercase fw-semibold mb-3">Email</label>
                                    <input type="text" name="email" value="{{ $filter['email'] ?? '' }}" class="form-control" id="email" placeholder="Serach By Email">
                                </div>
                                <div class="mb-4">
                                    <label for="mobile_no"
                                    ...
                                    <!--<select class="form-control" name="mobile_no"
                                        ...
                                        @if(isset($mobileNo) && count($mobileNo) > 0)
                                            @foreach($mobileNo as $key => $val)
                                                <option value="{{ $val->mobile_no }}" @if(isset($filter['mobile_no']) && $filter['mobile_no'] == $val->mobile_no) selected @endif>+{{ $val->country_code }}{{ $val->mobile_no }}</option>
                                            @endforeach
                                        @endif
                                    </select>-->
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" name="dob" value="{{ $filter['dob'] ?? '' }}" class="form-control flatpickr-input" id="datepicker-range"
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="row g-2">
                                        <div class="col-lg-6">
                                            <div class="form-check">
                                                <input class="form-check-input" name="gender" type="checkbox"
                                                    id="inlineRadio1" value="male" @if(isset($filter['gender']) && $filter['gender'] == 'male') checked @endif>
                                        ...
                                        <div class="col-lg-6">
                                            <div class="form-check">
                                                <input class="form-check-input" name="gender" type="checkbox"
                                                    id="inlineRadio2" value="female" @if(isset($filter['gender']) && $filter['gender'] == 'female') checked @endif>
                                                <label class="form-check-label"
                                ...
                                <div class="mb-4">
                                    ...
                                    {!! Form::select('area[]',$area ?? '',$filter['area'] ?? '',[ 'class'=>'form-control','id'=>'area','data-select2init'=>'true','multiple'=>'true','data-placeholder'=>'Please Select']) !!}
                                </div>
                                <div class="mb-4">
                                    <label for="country_id" class="form-label text-muted text-uppercase fw-semibold mb-3">Country</label>
                                    {!! Form::select('country_id[]',$country ?? '',$filter['country_id'] ?? '',[ 'class'=>'form-control','id'=>'country_id','data-select2init'=>'true','multiple'=>'true','data-placeholder'=>'Please Select']) !!}
                                </div>
                                <div class="mb-4">
                                    <label for="state_id" class="form-label text-muted text-uppercase fw-semibold mb-3">State</label>
                                    {!! Form::select('state_id[]',$state ?? [],$filter['state_id'] ?? [],[ 'class'=>'form-control','id'=>'state_id','data-select2init'=>'true','multiple'=>'true','data-placeholder'=>'Please Select']) !!}
                                </div>
                                <div class="mb-4">
                                    <label for="city_id" class="form-label text-muted text-uppercase fw-semibold mb-3">City</label>
                                    {!! Form::select('city_id[]',$city ?? [],$filter['city_id'] ?? [],[ 'class'=>'form-control','id'=>'city_id','data-select2init'=>'true','multiple'=>'true','data-placeholder'=>'Please Select']) !!}
                                </div>
                                <div class="mb-4">
                                    <label for="status-select"
                                        class="form-label text-muted text-uppercase fw-semibold mb-3">Marital Status</label>
                                    <div class="row g-2">
                                        <div class="col-lg-6">
                                            <div class="form-check">
                                                <input class="form-check-input" name="marital_status" type="checkbox"
                                                    id="inlineRadio1" value="1" @if(isset($filter['marital_status']) && $filter['marital_status'] == 'a') checked @endif>
                                                <label class="form-check-label"
                                        ...
                                        <div class="col-lg-6">
                                            <div class="form-check">
                                                <input class="form-check-input" name="marital_status" type="checkbox"
                                                    id="inlineRadio2" value="2" @if(isset($filter['marital_status']) && $filter['marital_status'] == '2') checked @endif>
                                                <label class="form-check-label"
                                ...
                                <div class="mb-4">
                                    <label for="datepicker-range"
                                        class="form-label text-muted text-uppercase fw-semibold mb-3">Marriage Date</label>
                                    <input type="text" name="marriage_date" value="{{ $filter['marriage_date'] ?? '' }}" class="form-control flatpickr-input" id="datepicker-range1"
                                        data-provider="flatpickr" data-range="true"
                                        placeholder="Select Marriage date">
                                </div>
                                <div class="mb-4">
                                    <label for="native_place"
                                        class="form-label text-muted text-uppercase fw-semibold mb-3">Native Place</label>
                                    <input type="text" name="native_place" value="{{ $filter['native_place'] ?? '' }}" class="form-control" id="native_place" placeholder="Serach By Native Place">
                                </div>
                                <div class="mb-4">
                                    <label for="industry_id" class="form-label text-muted text-uppercase fw-semibold mb-3">Business & Service Industry</label>
                                    {!! Form::select('industry_id[]',$industry ?? [],$filter['industry_id'] ?? [],[ 'class'=>'form-control','id'=>'industry_id','data-select2init'=>'true','multiple'=>'true','data-placeholder'=>'Please Select']) !!}
                                </div>
                                <div class="mb-4">
                                    <label for="pa_country_id" class="form-label text-muted text-uppercase fw-semibold mb-3">Country (Permanent Address)</label>
                                    {!! Form::select('pa_country_id[]',$country ?? '',$filter['pa_country_id'] ?? '',[ 'class'=>'form-control','id'=>'pa_country_id','data-select2init'=>'true','multiple'=>'true','data-placeholder'=>'Please Select']) !!}
                                </div>
                                <div class="mb-4">
                                    <label for="pa_state_id" class="form-label text-muted text-uppercase fw-semibold mb-3">State (Permanent Address)</label>
                                    {!! Form::select('pa_state_id[]',$state ?? [],$filter['pa_state_id'] ?? [],[ 'class'=>'form-control','id'=>'pa_state_id','data-select2init'=>'true','multiple'=>'true','data-placeholder'=>'Please Select']) !!}
                                </div>
                                <div class="mb-4">
                                    <label for="pa_city_id" class="form-label text-muted text-uppercase fw-semibold mb-3">City (Permanent Address)</label>
                                    {!! Form::select('pa_city_id[]',$city ?? [],$filter['pa_city_id'] ?? [],[ 'class'=>'form-control','id'=>'pa_city_id','data-select2init'=>'true','multiple'=>'true','data-placeholder'=>'Please Select']) !!}
                                </div>

                                <div class="mb-4">
                                    <label for="ca_country_id" class="form-label text-muted text-uppercase fw-semibold mb-3">Country (Communication Address)</label>
                                    {!! Form::select('ca_country_id[]',$country ?? '',$filter['ca_country_id'] ?? '',[ 'class'=>'form-control','id'=>'ca_country_id','data-select2init'=>'true','multiple'=>'true','data-placeholder'=>'Please Select']) !!}
                                </div>
                                <div class="mb-4">
                                    <label for="ca_state_id" class="form-label text-muted text-uppercase fw-semibold mb-3">State (Communication Address)</label>
                                    {!! Form::select('ca_state_id[]',$state ?? [],$filter['ca_state_id'] ?? [],[ 'class'=>'form-control','id'=>'ca_state_id','data-select2init'=>'true','multiple'=>'true','data-placeholder'=>'Please Select']) !!}
                                </div>
                                <div class="mb-4">
                                    <label for="ca_city_id" class="form-label text-muted text-uppercase fw-semibold mb-3">City (Communication Address)</label>
                                    {!! Form::select('ca_city_id[]',$city ?? [],$filter['ca_city_id'] ?? [],[ 'class'=>'form-control','id'=>'ca_city_id','data-select2init'=>'true','multiple'=>'true','data-placeholder'=>'Please Select']) !!}
                                </div>

                                <div class="mb-4">
                                    <label for="ma_country_id" class="form-label text-muted text-uppercase fw-semibold mb-3">Country (Magazine Address)</label>
                                    {!! Form::select('ma_country_id[]',$country ?? '',$filter['ma_country_id'] ?? '',[ 'class'=>'form-control','id'=>'ma_country_id','data-select2init'=>'true','multiple'=>'true','data-placeholder'=>'Please Select']) !!}
                                </div>
                                <div class="mb-4">
                                    <label for="ma_state_id" class="form-label text-muted text-uppercase fw-semibold mb-3">State (Magazine Address)</label>
                                    {!! Form::select('ma_state_id[]',$state ?? [],$filter['ma_state_id'] ?? [],[ 'class'=>'form-control','id'=>'ma_state_id','data-select2init'=>'true','multiple'=>'true','data-placeholder'=>'Please Select']) !!}
                                </div>
                                <div class="mb-4">
                                    <label for="ma_city_id" class="form-label text-muted text-uppercase fw-semibold mb-3">City (Magazine Address)</label>
                                    {!! Form::select('ma_city_id[]',$city ?? [],$filter['ma_city_id'] ?? [],[ 'class'=>'form-control','id'=>'ma_city_id','data-select2init'=>'true','multiple'=>'true','data-placeholder'=>'Please Select']) !!}
                                </div>

                                <div class="offcanvas-footer border-top p-3 text-center hstack gap-2">
                                    <a href="{{ url('admin/devotees') }}" class="btn btn-light w-100">Clear Filter</a>
                    ...
                    <div class="offcanvas offcanvas-end" tabindex="-1" id="exportModal" aria-labelledby="offcanvasExampleLabel">
                        ...
                        <form method="POST" action="{{url($actionURL.'/export')}}" class="d-flex flex-column justify-content-end h-100">
                            @csrf
                            <input type="hidden" name="filterData" value="{{json_encode($filter)}}">
                            <div class="offcanvas-body">
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        $(document).on('change', '#assign_category_id', function() {
            var val = $(this).val();
            $("#assignCategoryModal").modal('show');
            var values = $("input[name^='deleteuser']:checked").map(function (idx, ele) {
                return $(ele).val();
            }).get();
            $('#assignCategoryModal #multiple_userid').val(values);
            $('#assignCategoryModal #cat_id').val(val);
        });
        ...
        $('#example').dataTable({
            ...
            language: {
                info: "Showing 1 to {{count($devotee)}} of {{$totalDevotee}} entries"
            }
        });
        ...
        $(document).on('click','.delete_user_check',function(){
            ...
            $('#remove-actions').hide();
            $('#assign_category_id').hide();

            if(checked_length > 0){
                $('#remove-actions').show();
                $('#assign_category_id').show();
            }
        });
    ...
    <script type="text/javascript">
        $(function() {
            ...
            select2OptgroupCheckout('pa_country_id', '_country');
            select2OptgroupCheckout('pa_state_id', '_state');
            select2OptgroupCheckout('pa_city_id', '_city');
            $("#pa_country_id").change(function() {
                $("#pa_state_id").html("");
                var formData = new FormData();
                var selectedcountries = $(this).val();
                formData.append("country", JSON.stringify(selectedcountries));
                ajaxRequest("dropdown/country/states-options", formData).then(res => {
                    $("#pa_state_id").html(res.data.html);
                    select2OptgroupCheckout('pa_state_id', '_state');
                }).catch((err) => {
                    console.log(err);
                });
            });
            $("#pa_state_id").change(function() {
                $("#pa_city_id").html("");
                var formData = new FormData();
                var selectedcountries = $(this).val();
                formData.append("state", JSON.stringify(selectedcountries));
                ajaxRequest("dropdown/country/cities-options", formData).then(res => {
                    $("#pa_city_id").html(res.data.html);
                    select2OptgroupCheckout('pa_city_id', '_city');
                }).catch((err) => {
                    console.log(err);
                });
            ...
            select2OptgroupCheckout('ca_country_id', '_country');
            select2OptgroupCheckout('ca_state_id', '_state');
            select2OptgroupCheckout('ca_city_id', '_city');
            $("#ca_country_id").change(function() {
                $("#ca_state_id").html("");
                var formData = new FormData();
                var selectedcountries = $(this).val();
                formData.append("country", JSON.stringify(selectedcountries));
                ajaxRequest("dropdown/country/states-options", formData).then(res => {
                    $("#ca_state_id").html(res.data.html);
                    select2OptgroupCheckout('ca_state_id', '_state');
                }).catch((err) => {
                    console.log(err);
                });
            });
            $("#ca_state_id").change(function() {
                $("#ca_city_id").html("");
                var formData = new FormData();
                var selectedcountries = $(this).val();
                formData.append("state", JSON.stringify(selectedcountries));
                ajaxRequest("dropdown/country/cities-options", formData).then(res => {
                    $("#ca_city_id").html(res.data.html);
                    select2OptgroupCheckout('ca_city_id', '_city');
                }).catch((err) => {
                    console.log(err);
                });
            ...
            select2OptgroupCheckout('ma_country_id', '_country');
            select2OptgroupCheckout('ma_state_id', '_state');
            select2OptgroupCheckout('ma_city_id', '_city');
            $("#ma_country_id").change(function() {
                ...
                formData.append("country", JSON.stringify(selectedcountries));
                ajaxRequest("dropdown/country/states-options", formData).then(res => {
                    $("#ma_state_id").html(res.data.html);
                    select2OptgroupCheckout('ma_state_id', '_state');
                }).catch((err) => {
                    console.log(err);
                });
            });
            $("#ma_state_id").change(function() {
                $("#ma_city_id").html("");
                var formData = new FormData();
                var selectedcountries = $(this).val();
                formData.append("state", JSON.stringify(selectedcountries));
                ajaxRequest("dropdown/country/cities-options", formData).then(res => {
                    $("#ma_city_id").html(res.data.html);
                    select2OptgroupCheckout('ma_city_id', '_city');
                }).catch((err) => {
                    console.log(err);
                });
        ...
        $(document).on('click', '.btn-download', function (e) {
            window.open('{{route('devotee.downloadSample')}}', '_blank');
        });
